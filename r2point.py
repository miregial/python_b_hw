...
class R2Point:
    ...
    # Конструктор
    def __init__(self, x=None, y=None): # значения по умолчанию
        if x is None:
            x = float(input("x -> "))
        if y is None:
            y = float(input("y -> "))
    ...
    def f(self):  # значения sin(xy)
        return sin(self.x*self.y)

    # Площадь треугольника
    @staticmethod  # статические функциональные методы как ф(х)
    def area(a, b, c):
...
if __name__ == "__main__":
    ...
    #print(type(point), point.dict)
    print(point.dist(R2Point(1.0, 0.0)))
    a, b, c = R2Point(0.0, 0.0), R2Point(1.0, 0.0), R2Point(1.0, 1.0)
    print(R2Point.area(a, c, b))
    p1=R2Point(1, 0)
    print(p1.__dict__,p1.f())
